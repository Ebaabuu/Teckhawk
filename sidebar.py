...
def render_sidebar(show_share_link, rename_chat, confirm_delete):
    with st.sidebar:
        ...
        # New Chat Button
        st.button("➕      New Chat", use_container_width=True, on_click=lambda: st.session_state.update({"current_chat_id": None}))
        ...
        for cid, data in sorted_sessions:
            if search_q.lower() in data['title'].lower():
                # We use a container to allow the "Hover for Dots" CSS to work
                with st.container():
                    ...
                    # The Tool Menu (Three Dots) - Targeted by CSS in app.py
                    with st.popover(""):
                        ...
                        if data['pinned']:
                            if st.button("🚫 Unpin", key=f"p_{cid}", use_container_width=True):
                                st.session_state.chat_sessions[cid]['pinned'] = False; st.rerun()
                        else:
                            if st.button("📌 Pin Chat", key=f"p_{cid}", use_container_width=True):
                                st.session_state.chat_sessions[cid]['pinned'] = True; st.rerun()
